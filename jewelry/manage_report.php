...
<head>
    ...
    <style>
    body { font-family: 'Inter', sans-serif; background-color: #FBF9F6; color: #4D4C48; }
    .admin-sidebar { background-color: #4D4C48; }
    .nav-active { background-color: #8B7B61; color: white; }
    .table-header { background-color: #8B7B61; color: white; font-weight: 600; }
    .logout-btn { background-color: #DCCEB8; color: #4D4C48; font-weight: 600; transition: all 0.2s; }
    .logout-btn:hover { background-color: #8B7B61; color: white; }
    ...
    @media print {
        ...
        /* Hide UI Elements */
        .no-print, .admin-sidebar, #printModal, .logout-btn, .chart-container { 
        ...
        .print-stat-value { font-size: 16pt; font-weight: bold; }
        .print-stat-sub { font-size: 8pt; font-style: italic; color: #777; margin-top: 4px; }
        ...
        th {
            background-color: #eee !important; /* Override teal to gray for B&W printing */
            color: black !important;
        ...
        span[class*="rounded-full"] {
            ...
            color: black !important;
            background-color: transparent !important; /* Remove color backgrounds for clean B&W print */
        }
    }
    </style>
</head>
...
<body class="flex antialiased">
    ...
    <!-- Sidebar -->
    <div class="w-64 admin-sidebar h-screen p-6 fixed flex flex-col shadow-2xl z-50 no-print">
        ...
        <ul class="space-y-2 flex-grow">
            ...
            <li class="nav-active"><a href="manage_report.php" class="block p-3 rounded text-white nav-active transition-all">Sales Report</a></li>
        </ul>
    ...
    <div class="flex-1 ml-64 p-10">
        ...
        <!-- Stats Cards (Summary) -->
        <div id="screen-summary-cards" class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 print-section-summary no-print">
            <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
                <p class="text-sm font-semibold text-brand-subtext uppercase">Total Revenue</p>
                <p class="font-serif text-3xl font-extrabold text-brand-teal mt-2">₱<?= number_format($stats['total_revenue'], 2) ?></p>
            ...
            <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
                ...
                <p class="font-serif text-3xl font-extrabold text-yellow-600 mt-2"><?= $pendingStats['pending_count'] ?></p>
        ...
        <div class="bg-white p-6 rounded-xl shadow-lg mb-8 no-print border border-gray-200">
            <form method="GET" action="manage_report.php" class="flex flex-col md:flex-row items-end gap-4">
                <div class="flex-1">
                    ...
                    <input type="date" name="start_date" value="<?= $start_date ?>" class="w-full p-2 border border-gray-300 rounded-lg">
                </div>
                <div class="flex-1">
                    <label class="block text-sm font-semibold text-brand-dark mb-2">End Date</label>
                    <input type="date" name="end_date" value="<?= $end_date ?>" class="w-full p-2 border border-gray-300 rounded-lg">
                </div>
                <div class="flex-1">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="px-6 py-2 bg-brand-teal text-white rounded-lg font-semibold shadow-md">Apply Filter</button>
                    
                    <!-- PRINT BUTTON -->
                    <button type="button" onclick="window.print()" class="px-6 py-2 bg-gray-800 text-white border border-black rounded-lg font-semibold shadow-md hover:bg-black transition-colors">
                        Print Report
                    </button>
                </div>
            </form>
                        ...
                        <?php foreach ($topProducts as $index => $prod): ?>
                            <div class="flex items-center gap-4 p-3 bg-gray-50 rounded-lg border border-gray-100">
                                ...
                                <div class="flex-grow min-w-0">
                                    <p class="text-sm font-semibold truncate text-brand-dark"><?= htmlspecialchars($prod['Name']) ?></p>
                                    <p class="text-xs text-gray-500">Sold: <?= $prod['total_sold'] ?></p>
                                </div>
                                <!-- Revenue -->
                                <div class="text-right">
                                    <p class="text-sm font-bold text-brand-teal">₱<?= number_format($prod['total_product_revenue'], 2) ?></p>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
        ...
        <div id="screen-table-section" class="bg-white rounded-xl shadow-lg overflow-hidden print-section-table border border-gray-200">
                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <?php if (empty($recentOrders)): ?>
                            <tr><td colspan="5" class="px-6 py-10 text-center text-brand-subtext">No orders found for selected filters.</td></tr>
                        <?php else: ?>
                            <?php foreach ($recentOrders as $order): ?>
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 font-semibold text-brand-teal">₱<?= number_format($order['Total_Amount'], 2) ?></td>
                                    <td class="px-6 py-4">
                                        <?php 
                                        ...
                                        ?>
                                        <span class="px-2 py-1 text-xs font-semibold rounded-full <?= $statusColor ?>"><?= htmlspecialchars($order['Status']) ?></span>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        new Chart(ctx, {
            ...
            options: {
                ...
                scales: {
                    y: { type: 'linear', position: 'left', grid: { borderDash: [5, 5] } },
