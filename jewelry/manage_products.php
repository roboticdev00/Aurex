...
// Count products by category (For Filters)
 ...
 $earringsCount = count(array_filter($products, function($p) { return strtolower($p['Category_Name']) === 'earrings'; }));
 $braceletCount = count(array_filter($products, function($p) { return strtolower($p['Category_Name']) === 'bracelet'; }));
 $necklaceCount = count(array_filter($products, function($p) { return strtolower($p['Category_Name']) === 'necklace'; }));
 $ringsCount = count(array_filter($products, function($p) { return strtolower($p['Category_Name']) === 'rings'; }));

// Count Stock Status (For Pie Chart)
 $inStockCount = 0;
 $lowStockCount = 0;
 $outOfStockCount = 0;
...
<head>
    <meta charset="UTF-8">
    <title>Manage Products - Aurex Jewelry</title>
    ...
    <style>
        ...
        /* Row animation for Out of Stock */
        .row-out-of-stock {
            /* animation is handled by Tailwind class animate-pulse-red-strong */
        }
    </style>
</head>
<body class="flex antialiased">
    ...
    <div class="w-64 admin-sidebar h-screen p-6 fixed flex flex-col shadow-2xl z-50">
        ...
        <ul class="space-y-2 flex-grow">
            ...
            <li><a href="manage_products.php" class="block p-3 rounded text-white nav-active transition-all">Manage Products</a></li>
    ...
    <div class="flex-1 ml-64 p-10">
        <div class="flex justify-between items-center mb-6 border-b border-gray-200 pb-4">
            ...
            <a href="edit_product.php" class="bg-brand-teal text-white px-6 py-2 rounded-full font-semibold hover:bg-opacity-90 transition-colors shadow-md">
                + Add New Product
            </a>
        </div>
            ...
            <div class="md:col-span-3 flex flex-col gap-2 pt-2">
                ...
                <div class="filter-item active" data-filter="all">
                    <div class="flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-current" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100" id="productTableBody">
                    <?php if (empty($products)): ?>
                        <tr><td colspan="8" class="px-6 py-10 text-center text-brand-subtext text-lg">No products found in catalog. Start by adding a new one!</td></tr>
                    <?php else: ?>
                        ...
                        <?php 
                            ...
                            if ($product['Stock'] > 5) {
                                ...
                                $stockClass = 'bg-red-600 text-white';
                                $stockLabel = 'Out of Stock'; 
                                $rowClass = 'row-out-of-stock animate-pulse-red-strong';
                        ...
                        <tr data-category="<?= strtolower(htmlspecialchars($product['Category_Name'])) ?>" data-stock="<?= htmlspecialchars($product['Stock']) ?>" class="<?= $rowClass ?>">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-brand-dark"><?= htmlspecialchars($product['Product_ID']) ?></td>
                            
                            <!-- Image Column -->
                            <td class="px-6 py-4 whitespace-nowrap">
                                <img src="<?= $product_image_url ?>" alt="<?= htmlspecialchars($product['ProductName']) ?>" class="h-10 w-10 rounded-md object-cover shadow-sm border border-gray-200">
                            </td>
                            
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-brand-dark"><?= htmlspecialchars($product['ProductName']) ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-brand-subtext"><?= htmlspecialchars($product['Category_Name']) ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-brand-teal">₱<?= number_format($product['Price'], 2) ?></td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-brand-subtext capitalize <?= ($product['Stock'] == 0) ? 'text-red-600 font-bold' : '' ?>">
                                <?= htmlspecialchars($product['Availability']) ?>
                            </td>
                            
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-3">
                                <a href="edit_product.php?id=<?= $product['Product_ID'] ?>" class="px-3 py-1 bg-blue-500 text-white rounded text-xs font-semibold hover:bg-blue-600 transition-colors">
                                    Edit
                                </a>
                                <button class="delete-product-btn px-3 py-1 bg-red-500 text-white rounded text-xs font-semibold hover:bg-red-600 transition-colors" data-product-id="<?= $product['Product_ID'] ?>" data-product-name="<?= htmlspecialchars($product['ProductName']) ?>">
                                    Delete
                                </button>
                            </td>
                        </tr>
