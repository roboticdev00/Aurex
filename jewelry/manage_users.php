...
$adminUsersCount = count(array_filter($users, function ($u) {
    return $u['Role'] === 'admin';
}));
$customerUsersCount = count(array_filter($users, function ($u) {
    return $u['Role'] !== 'admin';
}));
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>Manage Users - Jewellery Admin</title>
    ...
    <style>
        ...
        .stats-card {
            ...
            /* THE "LITTLE BROWN" ON THE LEFT ONLY */
        ...
        .stats-card.active {
            background-color: #FBF9F6;
            /* brand-beige */
            /* Keep Left accent on active state */
            border-left: 5px solid #8B7B61;
...
<body class="flex antialiased">
    ...
    <div class="w-64 admin-sidebar h-screen p-6 fixed flex flex-col shadow-2xl z-20">
        ...
        <ul class="space-y-2 flex-grow">
            ...
            <li><a href="manage_users.php" class="block p-3 rounded text-white nav-active transition-all">
    ...
    <div class="flex-1 ml-64 p-10">
        ...
        <?php if ($message): ?>
            <div
                class="p-4 mb-4 rounded-xl <?= $message_type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' ?> border border-gray-200">
                <?= htmlspecialchars($message) ?>
            </div>
        <?php endif; ?>

        <!-- Statistics Cards (With Left Brown Accent Only) -->
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="stats-card active p-6 cursor-pointer relative overflow-hidden group" data-filter="all">
                <div class="flex items-start justify-between">
                    <div class="flex-1">
                        <h3
                            class="stats-label text-sm font-semibold text-gray-400 uppercase mb-1 group-hover:text-brand-teal transition-colors">
                            All Users</h3>
        ...
        <div class="bg-white p-6 rounded-xl shadow-lg overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100" id="userTableBody">
                    <?php if (empty($users)): ?>
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-brand-subtext text-lg">No users found.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($users as $user): ?>
                            <tr data-role="<?= htmlspecialchars($user['Role']) ?>">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-brand-dark">
                                    <?= htmlspecialchars($user['User_ID']) ?></td>
                                ...
                                <!-- Updated Action Buttons Styling (Pill Design) -->
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-3">
                                    ...
                                    <a href="manage_users.php?delete_id=<?= $user['User_ID'] ?>"
                                        ...
                                        class="px-3 py-1 bg-red-500 text-white rounded text-xs font-semibold hover:bg-red-600 transition-colors">
                                        Delete
                                    </a>
                                </td>
                            </tr>
