...
<head>
    <meta charset="UTF-8">
    <title>Manage Ratings - Jewellery</title>
    ...
    /* MODAL ANIMATIONS (From index_user.php) */
    .modal {
...
<body class="flex antialiased">
    ...
    <div class="w-64 admin-sidebar h-screen p-6 fixed flex flex-col shadow-2xl z-20">
        ...
        <ul class="space-y-2 flex-grow">
            ...
            <li><a href="manage_ratings.php" class="block p-3 rounded text-white nav-active transition-all">
    ...
    <div class="flex-1 ml-64 p-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div class="bg-white p-6 rounded-xl shadow-lg card-border-teal border-l-4">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p class="font-serif text-3xl font-extrabold text-brand-teal mt-1">
                            <?= number_format($stats['total_reviews']) ?></p>
                    </div>
            ...
            <div class="bg-white p-6 rounded-xl shadow-lg card-border-teal border-l-4">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p class="font-serif text-3xl font-extrabold text-brand-teal mt-1">
                            <?= number_format($stats['avg_rating'], 1) ?></p>
                    </div>
            ...
            <div class="bg-white p-6 rounded-xl shadow-lg card-border-teal border-l-4">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p class="font-serif text-3xl font-extrabold text-brand-teal mt-1">
                            <?= number_format($stats['five_star']) ?></p>
                    </div>
                    <div class="star-rating">
                        ★★★★★
                    </div>
                ...
                <div class="text-xs text-brand-subtext">
                    <?= $stats['total_reviews'] > 0 ? round(($stats['five_star'] / $stats['total_reviews']) * 100, 1) : 0 ?>%
                    of all reviews
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-xl shadow-lg card-border-teal border-l-4">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p class="font-serif text-3xl font-extrabold text-brand-teal mt-1">
                            <?= number_format($stats['one_star']) ?></p>
                    </div>
                    <div class="star-rating">
                ...
                <div class="text-xs text-brand-subtext">
                    <?= $stats['total_reviews'] > 0 ? round(($stats['one_star'] / $stats['total_reviews']) * 100, 1) : 0 ?>%
                    of all reviews
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div class="bg-white p-6 rounded-xl shadow-lg">
                ...
                <div class="space-y-3">
                    <?php for ($i = 5; $i >= 1; $i--): ?>
                    <?php
                        // FIXED: Changed array indexing to account for reverse loop
            ...
            <div class="bg-white p-6 rounded-xl shadow-lg">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                        <span
                            class="w-6 h-6 bg-brand-teal text-white text-xs font-bold rounded-full flex items-center justify-center">
                            <?= $index + 1 ?>
                        </span>
                        <img src="<?= htmlspecialchars($product['Images']) ?>"
                            alt="<?= htmlspecialchars($product['Name']) ?>" class="w-10 h-10 object-cover rounded">
                        <div class="flex-grow">
                            <p class="font-semibold text-brand-dark text-sm"><?= htmlspecialchars($product['Name']) ?>
                            </p>
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs text-brand-subtext"><?= number_format($product['avg_rating'], 1) ?>
                                    (<?= $product['review_count'] ?>)</span>
                            </div>
                        </div>
                        <button onclick="viewProduct(<?= $product['Product_ID'] ?>)"
                            class="px-3 py-1 text-sm bg-brand-teal text-white rounded hover:bg-opacity-90 transition-colors">
                            View
                        </button>
                    </div>
                    <?php endforeach; ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                ...
                <div>
                    ...
                    <select id="filterProduct"
                        class="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-teal bg-white">
                ...
                <div>
                    <button onclick="applyFilters()"
                        class="w-full bg-brand-teal text-white font-bold py-2 px-4 rounded-lg hover:bg-brand-dark transition-colors shadow-md">
                        Apply Filter
                    </button>
                </div>
            </div>
        ...
        <div class="bg-white p-6 rounded-xl shadow-lg">
            <div class="flex justify-between items-center mb-6">
                ...
                <p class="text-sm text-brand-subtext">
                    Showing <span id="visibleCount"><?= count($ratings) ?></span> of <span
                        id="totalCount"><?= $total_ratings ?></span> reviews
                </p>
            </div>
            ...
            <div id="ratingsList" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="rating-item hover-lift-card p-4 bg-white" data-product-id="<?= $rating['Product_ID'] ?>"
                    data-rating="<?= $rating['Rating'] ?>">
                    <div class="flex justify-between items-start mb-3">
                        <div>
                            <h4 class="font-bold text-brand-dark text-sm mb-1 truncate">
                                <?= htmlspecialchars($rating['ProductName']) ?></h4>
                            <p class="text-xs text-brand-subtext truncate">by
                                <?= htmlspecialchars($rating['UserName']) ?></p>
                        </div>
                        <div class="text-right">
                            ...
                            <p class="text-[10px] text-brand-subtext">
                                <?= date('M d, Y', strtotime($rating['Rating_Date'])) ?></p>
                        </div>
                    </div>

                    <div class="mb-3">
                        <p class="text-brand-text text-sm line-clamp-3">
                            <?= htmlspecialchars($rating['Review_Text'] ?? 'No review text provided.') ?></p>
                    </div>

                    <?php if (!empty($rating['Review_Image'])): ?>
                    <div class="mb-3">
                        <img src="<?= htmlspecialchars($rating['Review_Image']) ?>" alt="Review Image"
                            class="review-image w-full h-32 object-cover" onclick="openImageModal(this.src)">
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="grid grid-cols-2 gap-2 mt-4">
                        <button onclick="viewProduct(<?= $rating['Product_ID'] ?>)"
                            class="text-xs bg-gray-100 text-brand-dark py-2 rounded-lg hover:bg-gray-200 transition-colors">
                            View Product
                        </button>
                        <!-- View Order Button -->
                        <button
    ...
    <div id="order-details-modal"
        ...
        <div class="modal-panel relative w-full max-w-lg bg-white rounded-lg shadow-xl p-5 overflow-hidden">
            <button id="close-modal-btn" class="absolute top-3 right-3 text-gray-400 hover:text-gray-600 z-10">
